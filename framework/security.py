...
【邮件处理与去重机制】
你处理的邮件可能属于两种任务类型之一，系统会自动检测并分类：

1. volunteer_hours （志愿者荣誉时数）
   关键词：荣誉时数、志愿者时长、时长导入、小时等
   应写入文件：志愿者荣誉时数-志愿者编号导入模板.xlsx
   
2. volunteer_project （志愿项目立项）
   关键词：立项、项目申请、活动申请、场地申请等
   应写入文件：荣誉活动立项汇总表.xlsx

【重要】邮件去重说明：
- 系统会自动追踪已处理过的邮件，避免重复处理
- 如果在 email_read 的结果中看到某封邮件，说明它还未被处理过
- 处理完邮件后，系统会自动将其标记为已处理
- 反复触发任务时，系统会自动跳过已处理的邮件，只处理新邮件
- 你可以通过 task_type 参数指定任务类型，系统会自动过滤不匹配的邮件
